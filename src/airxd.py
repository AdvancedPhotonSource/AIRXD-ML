...
class ARIXD:
    ...
    def train(self, images, labels, include_data='random', training_split=30):
        if include_data == 'random':
            include_data = random.sample(range(0,int(training_split/100*len(images))))
        X = self.get_feature(images[include_data])
        y = self.get_label(lables[include_data])
        self.model.fit(X, y)
        return

    def predict(self, image):
        X = np.zeros(image.shape, len(self.no_of_features))
        ...
        return y_pred.reshape(image.shape[0], image.shape[1])
    ...
    def get_feature(self, images):
        shp = images.shape
        X = np.zeros((shp[0]*shp[1]*shp[2], len(self.no_of_features)))
